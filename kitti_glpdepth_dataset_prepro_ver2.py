...
'''
1. First Linear equation 통해 outlier 제거.
2. Misc데이터의 결측값 제거.
3. Zloc가 90미만인 데이터만 추출 (90 이상인 데이터의 개수가 적어서 학습에 문제 발생)
'''
mask1 = (glp_kitti_preprocessing_data['depth_mean']-0.0930*glp_kitti_preprocessing_data['zloc'])+1.116 > 0
mask2 = (glp_kitti_preprocessing_data['depth_mean']-0.1091*glp_kitti_preprocessing_data['zloc'])+1.5365 > 0
mask3 = (glp_kitti_preprocessing_data['depth_mean']-0.13*glp_kitti_preprocessing_data['zloc'])-2.5 < 0

mask4 = (mask1 & mask2 & mask3)           
misc_mask = (glp_kitti_preprocessing_data['zloc']<50) & (glp_kitti_preprocessing_data['depth_mean']>8) & (glp_kitti_preprocessing_data['class']=='Misc')

glp_kitti_preprocessing_data = glp_kitti_preprocessing_data[mask4]       
glp_kitti_preprocessing_data = glp_kitti_preprocessing_data[-misc_mask] # Misc 결측값 제거
glp_kitti_preprocessing_data = glp_kitti_preprocessing_data[glp_kitti_preprocessing_data['zloc'] < 90] # zloc 90 미만 pick
...
#glp_kitti_preprocessing_data.to_csv('./datasets/glp_kitti_data_1.csv', index=False)
glp_kitti_preprocessing_data.to_csv('./datasets/glp_kitti_data_2.csv', index=False)
print(len(glp_kitti_preprocessing_data))